...
class Utilities():
    ...
    def start_dnsmasq(self):
        subprocess.call(['dnsmasq', '-8', '/var/run/networkd-ci/test-dnsmasq-log-file',
                         '--log-queries=extra',  '--log-dhcp', '--pid-file=/var/run/networkd-ci/test-test-dnsmasq.pid',
                         '--conf-file=/dev/null', '--interface=veth-peer', '--enable-ra', '--dhcp-range=2600::10,2600::20',
                         '--dhcp-range=192.168.5.10,192.168.5.200', '-R', '--dhcp-leasefile=/var/run/networkd-ci/lease',
                         '--dhcp-option=26,1492', '--dhcp-option=option:router,192.168.5.1', '--dhcp-option=33,192.168.5.4,192.168.5.5'])

        time.sleep(10)

    def stop_dns_masq(self, pid_file):
    ...
    def search_words_in_file(self, word):
        if os.path.exists(dnsmasq_log_file):
            with open (dnsmasq_log_file, 'rt') as in_file:
...
class NetworkdNetworkDHCPClientTests(unittest.TestCase, Utilities):
    ...
    def setUp(self):
        self.link_remove(self.links)
        self.stop_dns_masq(dnsmasq_pid_file)
    ...
    def test_dhcp_client_listen_port(self):
        ...
        dh_server = dhcp_server("dhcp_server")
        dh_server.start()
